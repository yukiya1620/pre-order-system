 ...
 *
 * 共通基盤(demo-tutorial.js)や配達確認の回答API・通知処理
 * (farmer-delivery-confirmations.js)の業務ロジックには一切手を加えず、
 * この画面のステップ定義・農家ホームからの再開判定・農家ホームへの
 * 引き継ぎだけをここに閉じ込める。
 *
 * 第5-B・第6段階最終監査を経て、390px幅でtooltipが画面外に
 * はみ出す問題への対応として、最初の注文カードのスポットライト対象を
 * カード全体から注文内容のまとめ(confirmation-card__summary、日付・
 * 購入者・商品・お届け先)だけに縮小した(farmer-delivery-confirmations.js側の
 * DOM構造への最小限の追加。回答ボタンの並び・API・通知処理は無変更)。
 * 「配達できる」「日程変更」「数量変更」「キャンセル相談」等の回答ボタンは
 * スポットライト対象に含めず、チュートリアル側から実際にクリックする
 * ことも一切無い(利用者本人が押すかどうかは自由)。
 *
 * 範囲: 最初の注文カード(の注文内容まとめ)→「もどる」リンク、の2ステップ。
 */
(function () {
    ...
    var steps = [
        {
            ...
            description: '注文内容をここで確認できます。この画面では、配達可否や日程変更などの対応も行えますが、今回は内容を見るだけにします。'
        },
        {
