...
| `DEMO_TUTORIAL_BUYER_PHONE`     | `00000000001`(上記デモ購入者電話番号のいずれか1件)             | 操作チュートリアルが購入者向けに案内するデモ電話番号。上記`DEMO_SMS_PHONE_NUMBERS`に含まれる番号を指定する。未設定の場合、チュートリアルは電話番号の案内を表示しない(チュートリアル自体は動作する) |
| `DEMO_TUTORIAL_FARMER_EMAIL`    | `demo-farmer@example.com`                                       | 操作チュートリアルが販売者向けに案内するデモ農家のメールアドレス。`PortfolioDemoSeeder`が作成する農家アカウントと一致させる。未設定の場合、「📋 販売者デモのログイン情報を入力」ボタンが表示されない |
| `DEMO_TUTORIAL_FARMER_PASSWORD` | README.md記載のデモ農家パスワード                                | 上記デモ農家アカウントのパスワード。Git・このドキュメントに書かない                                                        |
...
`config:cache`を実行した状態で`.env`を変更しても反映されません。`.env`の内容を確定させてから`config:cache`を実行する順序(上記の並び)を守ってください。運用開始後に`.env`を修正する場合は、`php artisan optimize:clear`でキャッシュを解除してから反映し直してください(README.mdの「本番環境での注意事項」と同じ注意点です)。この注意点は`DEMO_TUTORIAL_*`の3変数を後から追加・変更する場合も同様です。
...
- [ ] 購入者トップで「使い方を見る」ボタン、および初回アクセス時の自動チュートリアル開始が機能する
- [ ] 購入者向けチュートリアルにデモ電話番号(`DEMO_TUTORIAL_BUYER_PHONE`)が案内される
- [ ] 農家ログイン画面に「📋 販売者デモのログイン情報を入力」ボタンが表示され、押すとメール・パスワード欄に自動入力される
- [ ] 農家ホーム初回アクセス時に、販売者向けチュートリアルが自動で開始する
- [ ] 購入者・農家それぞれのチュートリアルを最後まで進め、「完了」まで到達できる
