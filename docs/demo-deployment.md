...
| キューワーカー | 不要(`QUEUE_CONNECTION=sync`または`database`のままで問題ない同期処理のみ) |
...
| `QUEUE_CONNECTION` | `sync`または`database`のままでよい | キューワーカー不要の前提と一致 |
...
## 7. 商品画像について(人間側での確認が必要な項目)

`database/seeders/demo-assets/products/` に格納されている以下6枚の画像について、コードや過去のコミット履歴(`git log`)を調査しましたが、**出典・利用権に関する記載がリポジトリ内に見当たりませんでした**。一般公開の前に、加藤さんご自身で出典・利用規約(商用利用可否・クレジット表記の要否等)をご確認いただく必要があります。私の判断で画像を差し替えることはしていません。
...
補足調査結果:
- 画像ファイルにEXIF/IPTCメタデータ(撮影者・著作権情報等)は含まれていません(確認済み)。
- 全て1200×900pxで統一されており、フリー素材サイト由来の可能性がありますが、断定はできません。
- 追加した際のコミット(`feat: add portfolio demo product images`)にも出典の記載はありませんでした。
