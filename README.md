...
1. 農家ホーム(F1)を開き、「💬 要対応(変更相談)」に `2` と表示されることを確認する
2. その項目を押すと、予約一覧(F3)が「相談あり」で絞り込まれた状態で開き、注文番号 `DEMO-0001` と `DEMO-0010` が表示される
3. `DEMO-0001`(デモ 太郎様)を開く(F4)と、「購入者からのご相談」欄に「数量変更」相談(2本→1本)が表示される
